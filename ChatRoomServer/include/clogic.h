...
public:
    ...
    /******************* IM ************************/
    void slot_addFriend(int clientfd,int userid);
    void slot_addGroup(int clientfd,int userid);
    void slot_getMessages(int clientfd,int userid);
    void slot_getGroupMessages(int clientfd,int userid);
    ...
    //离线通知
    void OffLineRq(sock_fd clientfd, char*szbuf, int nlen);
    //删除好友请求
    void DelFriendRq(sock_fd clientfd, char*szbuf, int nlen);

    //群聊
    //聊天请求
    void GroupChatRq(sock_fd clientfd, char*szbuf, int nlen);
    void GroupCreateRq(sock_fd clientfd, char*szbuf, int nlen);
    void GroupAddRq(sock_fd clientfd, char*szbuf, int nlen);
    void GroupAddRs(sock_fd clientfd, char*szbuf, int nlen);
    void GroupDelRq(sock_fd clientfd, char*szbuf, int nlen);
    void ImSendFileRq(sock_fd clientfd, char*szbuf, int nlen);
    void ImSendFileRs(sock_fd clientfd, char*szbuf, int nlen);
    void ImSendFileContentRq(sock_fd clientfd, char*szbuf, int nlen);
    void ImSendFileContentRs(sock_fd clientfd, char*szbuf, int nlen);
...
private:
    ...
    //userid -> sockfd
    map<int ,int> m_mapUseridToSockfd;
};
